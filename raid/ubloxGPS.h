...
//ON  = 0x01
//
byte settingsArray[] = {0x04, 0xFA, 0x00, 0x00, 0x4B, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void sendUBX(byte *UBXmsg, byte msgLength) {
  for(int i = 0; i < msgLength; i++) {
    GPS_SERIAL_PORT.write(UBXmsg[i]);
    GPS_SERIAL_PORT.flush();
  }
  GPS_SERIAL_PORT.println();
  GPS_SERIAL_PORT.flush();
}
...
byte getUBX_ACK(byte *msgID) {
  ...
  while (1) {
    if (GPS_SERIAL_PORT.available()) {
      incoming_char = GPS_SERIAL_PORT.read();
...
void setBaud(byte baudSetting) {
  if (baudSetting == 0x12) GPS_SERIAL_PORT.begin(4800);
  if (baudSetting == 0x4B) GPS_SERIAL_PORT.begin(19200);
  if (baudSetting == 0x96) GPS_SERIAL_PORT.begin(38400);
  if (baudSetting == 0xE1) GPS_SERIAL_PORT.begin(57600);
  if (baudSetting == 0xC2) GPS_SERIAL_PORT.begin(115200);
  if (baudSetting == 0x84) GPS_SERIAL_PORT.begin(230400);
}
...
void configureUblox(byte *settingsArrayPointer) {
  ...
  //if (gpsSetSuccess == 3) return false;
  gpsSetSuccess = 0;
  while(gpsSetSuccess < 3) {
